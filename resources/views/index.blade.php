    ...
    <div class="container mw-1620 bg-white border-radius-10">
      ...
      <section class="category-carousel container">
        ...
        <div class="swiper-wrapper">
            ...
            <div class="swiper-slide">
                <a href="{{ route('shop.index', ['categories' => $category->id]) }}" class="d-block text-center text-decoration-none">
                    <img loading="lazy" class="w-100 h-auto mb-3"
                        src="{{ asset('uploads/categories') }}/{{ $category->image }}"
                        width="124" height="124" alt="{{ $category->name }}" />
                    <div>
                        <span class="menu-link fw-medium">{{ $category->name }}</span>
                    </div>
                </a>
            </div>
            @endforeach
                  ...
                  <div class="swiper-slide product-card product-card_style3">
                    <div class="pc__img-wrapper">
                      <a href="{{ route('shop.product.details', ['product_slug'=>$sproduct->slug]) }}">
                        <img loading="lazy" src="{{ asset('uploads/products/thumbnails') }}/{{ $sproduct->image }}" width="258" height="313"
                    ...
                    <div class="pc__info position-relative">
                      ...
                      <div class="product-card__price d-flex">
                        <span class="money price text-secondary">
                            ...
                            <s>IDR{{ number_format($sproduct->regular_price, 0, ',', '.') }}</s>
                            IDR{{ number_format($sproduct->sale_price, 0, ',', '.') }}
                            @else
                            IDR{{ number_format($sproduct->regular_price, 0, ',', '.') }}
                            @endif
                        </span>
                      </div>
                    </div>
      ...
      <section class="products-grid container">
        ...
        <div class="row">
          ...
          <div class="col-6 col-md-4 col-lg-3">
            <div class="product-card product-card_style3 mb-md-4 mb-xxl-5">
                ...
                <div class="pc_info position-relative">
                    <h6 class="pc_title pt-3"><a href="{{route('shop.product.details', ['product_slug'=>$fproduct->slug])}}">{{$fproduct->name}}</a></h6>
                    <div class="product-card_price d-flex align-items-center">
                      <span class="money price text-secondary">
                            @if ($fproduct->sale_price)
                            <s>IDR{{ number_format($fproduct->regular_price, 0, ',', '.') }}</s>
                            IDR{{ number_format($fproduct->sale_price, 0, ',', '.') }}
                            @else
                            IDR{{ number_format($fproduct->regular_price, 0, ',', '.') }}
                            @endif
                    </span>
                    </div>
                </div>
